...
export def Editor(bufnr: number = 0, tcnum: number = 0): void
  ...
  win_execute(input_win, 'setlocal statusline=Input\ ' .. tcnum)
  win_execute(output_win, 'setlocal statusline=Output\ ' .. tcnum)
  # TODO: Airline.vim Plugin
...
def GetBorderChars(style: string): list<string>
  if style == 'double'
    return ['═', '║', '═', '║', '╔', '╗', '╝', '╚']
...
export def Picker(bufnr: number, tctbl: dict<any>, title: string,
  ...
  var popup = popup_menu(menu_items, {
    ...
    border: [],
    borderchars: GetBorderChars(config.floating_border),
